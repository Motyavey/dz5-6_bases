...
def setup_table():
    ...
    conn.commit()
    return jsonify({"message": "Таблиця створена успішно з індексами"})
...
def insert_data():
    ...
    conn.commit()
    return jsonify({"message": "Дані успішно вставлено"})
...
def update_data():
    ...
    conn.commit()
    return jsonify({"execution_plan": plan, "message": "Дані успішно оновлено"})
...
def delete_data():
    ...
    conn.commit()
    return jsonify({"execution_plan": plan, "message": "Дані успішно видалено"})
